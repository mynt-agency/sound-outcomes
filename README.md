...
- The two lead-capture forms (hero + close) submit to HubSpot via the Forms
  Submissions API (`src/lib/hubspot.ts`), then render the confirmation state.
  The portal ID and form GUID default to the production values but can be
  overridden with `NEXT_PUBLIC_HUBSPOT_PORTAL_ID` and
